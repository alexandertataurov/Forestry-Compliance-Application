...
class SimpleMessageBus:
    ...
    def __init__(self, db_path: str = "data/db/messages.db"):
        # Resolve DB path relative to project root so we don't depend on CWD
        self.db_path = self._resolve_db_path(db_path)
        self.agents: Dict[str, Agent] = {}
        self._lock = threading.RLock()
        self._init_db()

    def _resolve_db_path(self, path_str: str) -> Path:
        """Resolve the database path to an absolute path.

        - If a relative path is provided, make it relative to the project root
          (two levels up from this file: src/core -> project root).
        ...
        if not p.is_absolute():
            ...
            p = base_dir / p
        # Make sure full parent tree exists (data/db even if data missing)
